...
Fiskl has always recorded changes. What it did not do was show them to you. The audit log now surfaces that history in the interface: what changed, when, and who changed it. You will find it in your settings.

The part we think you will use most is the email side of it. When a client says they never received an invoice, you can now see what happened to that email — whether it was delivered, bounced, or rejected, and the reason why. Previously the honest answer was that we could not tell you without going and looking. Now you can look yourself.

[Audit Log](/settings/audit-log) explains what is recorded and how to filter it.
...
The public API has gained more methods. We will have more to say shortly about general MCP availability and the Claude integration, which is close enough that it deserves its own announcement rather than a paragraph here. [Public API](/integrations/public-api/overview) has the current documentation.
...
[Time tracking](/time-and-mileage/time-tracking) covers how the timer works.
